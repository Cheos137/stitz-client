...
public class Lpc
{
    ...
    throws IllegalArgumentException
    {
        ...
        for (int k = 0; k <= 159; k++)
        {
            temp = GSMMath.GSM_ADD(so[k], (short) 0);
            if (temp > smax)
                smax = temp;
        }
        ...
        else
        {
            ...
            scalauto = (short) (4 - GSMMath.gsm_norm(smax << 16)); /*
                                                                        * sub(4,.
                                                                        * .)
                                                                        */
        }
        ...
        if (scalauto > 0)
        {
            ...
            case 1:
                for (int k = 0; k <= 159; k++)
                {
                    so[k] = GSMMath.GSM_MULT_R(so[k], (short) 16384);
                }
                break;
            ...
            case 2:
                for (int k = 0; k <= 159; k++)
                {
                    so[k] = GSMMath.GSM_MULT_R(so[k], (short) (16384 >> 1));
                }
                break;
            ...
            case 3:
                for (int k = 0; k <= 159; k++)
                {
                    so[k] = GSMMath.GSM_MULT_R(so[k], (short) (16384 >> 2));
                }
                break;
            ...
            case 4:
                for (int k = 0; k <= 159; k++)
                {
                    so[k] = GSMMath.GSM_MULT_R(so[k], (short) (16384 >> 3));
                }
                break;
            }
        }
            ...
            throws IllegalArgumentException
    {
        ...
        for (int n = 1; n <= 8; n++, r_index++)
        {
            ...
            r[r_index] = GSMMath.gsm_div(temp, P[0]);
             ...
             * Schur recursion
             */
            temp = GSMMath.GSM_MULT_R(P[1], r[r_index]);
            P[0] = GSMMath.GSM_ADD(P[0], temp);

            for (int m = 1; m <= 8 - n; m++)
            {
                temp = GSMMath.GSM_MULT_R(K[m], r[r_index]);
                P[m] = GSMMath.GSM_ADD(P[m + 1], temp);

                temp = GSMMath.GSM_MULT_R(P[m + 1], r[r_index]);
                K[m] = GSMMath.GSM_ADD(K[m], temp);
            }
        }
    }
            ...
            throws IllegalArgumentException
    {
        ...
        for (int i = 0; i < 8; i++)
        {
            ...
            if (r[i] == GSMDef.MIN_WORD)
            {
                throw new IllegalArgumentException(
                        "Transformation_to_Log_Area_Ratios: r[" + i + "] = "
                                + r[i] + " should not be = " + GSMDef.MIN_WORD);
            }
        }
    }
    ...
    private void STEP2(int A, int B, int MAC, int MIC, short[] LAR, int index)
    {
        ...
        temp = GSMMath.GSM_MULT((short) A, LAR[index]);
        temp = GSMMath.GSM_ADD(temp, (short) B);
        temp = GSMMath.GSM_ADD(temp, (short) 256);
