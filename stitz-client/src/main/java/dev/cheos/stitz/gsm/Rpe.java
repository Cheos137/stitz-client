...
public class Rpe {
	...
	public void Gsm_RPE_Encoding(short[] e, /* -5..-1][0..39][40..44 IN/OUT */
		...
		/*
		 * Sets up xmc (13 array locations starting from xMc_index), xmaxc (one array
		 * location at xmaxc_Mc_index), exp_in and mant_in
		 */
	...
	private void Weighting_filter(short[] e) /* signal [-5..0.39.44] IN */
	{
		...
		/*
		 * (e[-5..-1] and e[40..44] are allocated by the caller, are initially zero and
		 * are not written anywhere.)
		 * 
		 * e -= 5;
		 * 
		 * The above case is true with the C code, although java does not have pointers
		 * so e[0..49] is all set for this method.
		 */
		...
		for (int k = 0; k <= 39; k++) {
			...
			/*
			 * Every one of these multiplications is done twice -- but I don't see an
			 * elegant way to optimize this. Do you?
			 */
			...
			/* + STEP( 2, 0 ) no sense in adding zero */
					+ (e[k + 3] * 2054) + (e[k + 4] * 5741) + (e[k + 5] * 8192) + (e[k + 6] * 5741) + (e[k + 7] * 2054)
					/* + STEP( 8, 0 ) no sense in adding zero */
					+ (e[k + 9] * -374) + (e[k + 10] * -134);
					
			/*
			 * 2 adds vs. >>16 => 14, minus one shift to compensate for those we lost when
			 * replacing L_MULT by '*'.
			 */
			
			L_result = GSMMath.SASR(L_result, 13);
			x[k] = (short) ((L_result < GSMDef.MIN_WORD ? GSMDef.MIN_WORD
					: (L_result > GSMDef.MAX_WORD ? GSMDef.MAX_WORD : L_result)));
		}
	}
	
	/*
	 * The signal x[0..39] is used to select the RPE grid which is represented by
	 * Mc.
	 */
	private void RPE_grid_selection(short[] xM, /* [0..12] OUT */
		...
		L_result += STEP(0, 1) + STEP(0, 2) + STEP(0, 3) + STEP(0, 4) + STEP(0, 5) + STEP(0, 6) + STEP(0, 7)
				+ STEP(0, 8) + STEP(0, 9) + STEP(0, 10) + STEP(0, 11) + STEP(0, 12);
		...
		/* i = 1 */
		L_result = 0;
		L_result += STEP(1, 0) + STEP(1, 1) + STEP(1, 2) + STEP(1, 3) + STEP(1, 4) + STEP(1, 5) + STEP(1, 6)
				+ STEP(1, 7) + STEP(1, 8) + STEP(1, 9) + STEP(1, 10) + STEP(1, 11) + STEP(1, 12);
		...
		/* i = 2 */
		L_result = 0;
		L_result += STEP(2, 0) + STEP(2, 1) + STEP(2, 2) + STEP(2, 3) + STEP(2, 4) + STEP(2, 5) + STEP(2, 6)
				+ STEP(2, 7) + STEP(2, 8) + STEP(2, 9) + STEP(2, 10) + STEP(2, 11) + STEP(2, 12);
	...
	private void APCM_quantization(short[] xM, /* [0..12] IN */
		...
		for (int i = 0; i <= 5; i++) {
			...
			if (!(exp <= 5)) {
				throw new IllegalArgumentException(
						"APCM_quantization: exp = " + exp + " is out of range. Should be <= 5");
			}
		...
		if (!(exp <= 6 && exp >= 0)) {
			throw new IllegalArgumentException(
					"APCM_quantization: exp = " + exp + " is out of range. Should be >= -4 and <= 6");
		}
		...
		if (!(temp <= 11 && temp >= 0)) {
			throw new IllegalArgumentException(
					"APCM_quantization: temp = " + temp + " is out of range. Should be >= 0 and <= 11");
		}
		
		xmaxc = GSMMath.GSM_ADD(GSMMath.SASR(xmax, temp), (short) (exp << 3));
		...
		if (!(exp <= 4096 && exp >= -4096)) {
			throw new IllegalArgumentException(
					"APCM_quantization: exp = " + exp + " is out of range. Should be >= -4096 and <= 4096");
		}
		if (!(mant >= 0 && mant <= 7)) {
			throw new IllegalArgumentException(
					"APCM_quantization: mant = " + mant + " is out of range. Should be >= 0 and <= 7");
		}
		...
		for (int i = 0; i <= 12; i++) {
			if (!(temp1 >= 0 && temp1 < 16)) {
				throw new IllegalArgumentException(
						"APCM_quantization: temp = " + temp + " is out of range. Should be >= 0 and < 16");
			}
			
			temp = (short) (xM[i] << temp1);
			temp = GSMMath.GSM_MULT(temp, temp2);
	...
	public void APCM_quantization_xmaxc_to_exp_mant(short xmaxc_elem, int METHOD_ID) throws IllegalArgumentException {
		...
		if (exp < -4 || exp > 6) {
			throw new IllegalArgumentException(
					"APCM_quantization_xmaxc_to_exp_mant: exp = " + exp + " is out of range. Should be >= -4 and <= 6");
		}
		if (mant < 0 || mant > 7) {
			throw new IllegalArgumentException("APCM_quantization_xmaxc_to_exp_mant: mant = " + mant
	...
	public void Gsm_RPE_Decoding_java(short xmaxc_elem, /*
														 * From Gsm_Decoder xmaxc short array
														 */
	 ...
	 * obtain the xMp[0..12] array. Table 4.6 is used to get the mantissa of xmaxc
	 * (FAC[0..7]).
	 */
	public void APCM_inverse_quantization(short[] xmc, /* [0..12] IN */
		...
		if (METHOD_ID == ENCODE) {
			...
			temp2 = GSMMath.GSM_SUB((short) 6, exp_in);
		} else { /* DECODE */
			temp1 = GSMDef.gsm_FAC[mant_out];
			temp2 = GSMMath.GSM_SUB((short) 6, exp_out);
		}
		temp3 = GSMMath.gsm_asl((short) 1, GSMMath.GSM_SUB(temp2, (short) 1));
		...
		for (int i = 0; i < 13; i++) {
			...
			if (!(temp <= 7 && temp >= -7)) { /* 4 bit signed */
				throw new IllegalArgumentException(
						"APCM_inverse_quantization: temp = " + temp + " is out of range. Should be >= -7 and <= 7");
			}
			temp <<= 12; /* 16 bit signed */
			temp = GSMMath.GSM_MULT_R(temp1, temp);
			temp = GSMMath.GSM_ADD(temp, temp3);
			xMp[xMp_point++] = GSMMath.gsm_asr(temp, temp2);
		}
	}
	
	/*
	 * This method computes the reconstructed long term residual signal ep[0..39]
	...
	public static void RPE_grid_positioning(short Mc, /* grid position IN */
		...
		if (!(0 <= Mc && Mc <= 3)) {
			throw new IllegalArgumentException(
					"RPE_grid_positioning: Mc = " + Mc + " is out of range. Should be >= 0 and <= 3");
		}
