...
public class LongTerm {
	public void Gsm_Long_Term_Predictor(short[] d, /* [0..39] residual signal IN */
			...
			int Nc_bc_index) {
		Calculation_of_the_LTP_parameters(d, k, dp, dp_dpp_point_dp0, bc, Nc, Nc_bc_index);
		Long_term_analysis_filtering(bc[Nc_bc_index], Nc[Nc_bc_index], dp, d, k, dpp, e, dp_dpp_point_dp0);
	}
	...
	private void Calculation_of_the_LTP_parameters(short[] d, /* [0..39] IN */
		...
		if (dmax == 0) {
			...
			if (!(dmax > 0)) {
				throw new IllegalArgumentException(
						"Calculation_of_the_LTP_parameters: dmax = " + dmax + " should be > 0.");
			}
			temp = GSMMath.gsm_norm(dmax << 16);
		}
		...
		if (!(scal >= 0)) {
			throw new IllegalArgumentException(
					"Calculation_of_the_LTP_parameters: scal = " + scal + " should be >= 0.");
		}
		...
		if (!(scal <= 100 && scal >= -100)) {
			throw new IllegalArgumentException(
					"Calculation_of_the_LTP_parameters: scal = " + scal + " should be >= -100 and <= 100.");
		}
		...
		if (!(Nc <= 120 && Nc >= 40)) {
			throw new IllegalArgumentException(
					"Calculation_of_the_LTP_parameters: Nc = " + Nc + " should be >= 40 and <= 120.");
		}
		...
		temp = GSMMath.gsm_norm(L_power);
		
		R = GSMMath.SASR(L_max << temp, 16);
		S = GSMMath.SASR(L_power << temp, 16);
		...
		for (int bc = 0; bc <= 2; bc++) {
			if (R <= GSMMath.GSM_MULT(S, GSMDef.gsm_DLB[bc])) {
				break;
			}
	...
	/*
	 * In this part, we have to decode the bc parameter to compute the samples of
	 * the estimate dpp[0..39]. The decoding of bc needs the use of table 4.3b. The
	 * long term residual signal e[0..39] is then calculated to be fed to the RPE
	 * encoding section.
	 */
	static void Long_term_analysis_filtering(short bc, /* IN */
		...
		switch (bc) {
			case 0:
				BP = (short) 3277;
				for (int k = 0; k <= 39; k++) {
					dpp[k + dp_dpp_index] = GSMMath.GSM_MULT_R(BP, dp[k - Nc + dp_dpp_index]);
					e[k + 5] = GSMMath.GSM_SUB(d[k + d_index], dpp[k + dp_dpp_index]);
				}
				break;
	 ...
	 */
	public void Gsm_Long_Term_Synthesis_Filtering(GSMState S, short Ncr, short bcr, short[] erp, /* [0..39] IN */
	 ...
	 */
		...
		if (!(Nr >= 40 && Nr <= 120)) {
			throw new IllegalArgumentException(
					"Gsm_Long_Term_Synthesis_Filtering Nr = " + Nr + " is out of range. Should be >= 40 and <= 120");
		}
		...
		if (brp == GSMDef.MIN_WORD) {
			throw new IllegalArgumentException("Gsm_Long_Term_Synthesis_Filtering brp = " + brp
					+ " is out of range. Should be = " + GSMDef.MIN_WORD);
		}
		
		for (int k = 0; k <= 39; k++) {
			drpp = GSMMath.GSM_MULT_R(brp, drp[k - Nr + dp0_index_start_drp]);
			drp[k + dp0_index_start_drp] = GSMMath.GSM_ADD(erp[k], drpp);
		}
